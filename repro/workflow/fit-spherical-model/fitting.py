...
from geocitmodel.loss import TripletLoss, SimilarityMetrics, NodeCentricTripletLoss
...
if symmetric:
    model = SphericalModel(
        n_nodes=n_nodes, dim=dim, c0=model_cmin, fit_embedding=geometry
    )
else:
    model = AsymmetricSphericalModel(
        n_nodes=n_nodes, dim=dim, c0=model_cmin, fit_embedding=geometry
    )
...
loss_func = TripletLoss(
    ...
    uniform_negative_sampling=False,
    kappa_regularization=5e-5,  # Changed from 1e-5 to 5e-5 2023-11-07
    # kappa_regularization=1e-5,  # Changed from 1e-4 to 1e-5 2023-07-16
    # kappa_regularization = 1e-4, # Changed from 2e-5 to 4e-5 2023-07-02
    # kappa_regularization = 2e-5, # Changed from 1e-5 to 2e-5 2023-06-26
)
